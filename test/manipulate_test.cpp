...
TEST(manipulate_test, clear_line) {
  EXPECT_STREQ(Foxterm::clear_line_end(), "\033[K");
  EXPECT_STREQ(Foxterm::clear_line_start(), "\033[1K");
  EXPECT_STREQ(Foxterm::clear_line(), "\033[2K");
}
TEST(manipulate_test, clear_screen) {
  EXPECT_STREQ(Foxterm::clear_screen_end(), "\033[J");
  EXPECT_STREQ(Foxterm::clear_screen_start(), "\033[1J");
  EXPECT_STREQ(Foxterm::clear_screen(), "\033[2J");
  EXPECT_STREQ(Foxterm::clear_screen_buffer(), "\033[3J");
}
...
TEST(manipulate_test, aux_port) {
  EXPECT_STREQ(Foxterm::serial_port_on(), "\033[5i");
  EXPECT_STREQ(Foxterm::serial_port_off(), "\033[4i");
}
